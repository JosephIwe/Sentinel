...
- [ ] `DnssecConnector` — **implemented, awaiting merge** on branch `feature/dnssec`. `src/connectors/dnssec.ts`. Builds and parses DNS messages directly (RFC 1035 wire format, RFC 4034 records, EDNS0 DO bit) because Node's `dns` module cannot request DS/DNSKEY/NSEC3PARAM. Reports DS and DNSKEY records, key tags computed per RFC 4034 Appendix B, validation status from the resolver's AD flag, and the NSEC/NSEC3 denial scheme. 33 tests. **Needs a one-line renumber to section 15 (Recommendations → 16) before merge**, since it was cut from a `main` that ended at section 13.
- [ ] `ShodanConnector`
