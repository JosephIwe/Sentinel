...
_Last reviewed: 2026-07-30, after implementing the Crawl4AI Web Footprint connector on branch `feature/crawl4ai-web-footprint` (unmerged; `main` at `71af270`). `docs/KNOWN_ISSUES.md` and `docs/MILESTONES.md` were reconciled against this file on 2026-07-28 and have not been revisited since; the connector-expansion entries below are newer than those two files._
...
- `npm run test`: **547/547 tests pass** across 31 files (269 → 547 as the v1.1 connectors landed: Certificate Transparency +21, ASN / IP Intelligence +30, RDAP +34, Reverse DNS +31, HTTP Security Headers +38, DNSSEC +33, Shodan +41, Web Footprint +50). The Web Footprint total is on its unmerged branch.
...
**Web Footprint** (Crawl4AI) is implemented on `feature/crawl4ai-web-footprint` and awaiting review — twelve merged, thirteen written.

Two connectors require configuration: Shodan (`SHODAN_API_KEY`) and Web Footprint (`CRAWL4AI_URL`). Unconfigured, both are inert: `NO_DATA` with a "not configured" diagnostic, no request made, and the report renders them as NOT CONFIGURED rather than a failure.
